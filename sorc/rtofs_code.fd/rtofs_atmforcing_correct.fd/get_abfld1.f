      subroutine get_abfld1(iunit,ip1,field1,idm_,jdm_,
     $     label1,level1,fln_name,ihead,time,span)
...
cjl           read (cline(i+1:),*) level,ltime,hminb,hmaxb
          read (cline(i+1:),*) time,span,hminb,hmaxb
          level=0 ! this is because we're reading a surface file.
         endif       
         endif
         if ( level.eq.level1) then
             nset=nset+1
             call zaiord(field1,ip1,.true., hmina,hmaxa,
     &              iunit)
c
