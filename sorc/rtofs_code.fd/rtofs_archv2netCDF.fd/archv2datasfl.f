...
c ---           how many columns are in this latitude band?
                cross = 1.0
                do jc= j-1,1,-1
                  if     (jplat(i,jc).ne.jlat) then
                    exit
                  endif
                  if     (ip(i,jc).eq.1) then
                    cross = cross + 1.0
                  endif
                enddo
                do jc= j+1,jj
                ...
                enddo
                fluxu = 0.5*(thicku(i,j  ,k)*scuy(i,j  )*u(i,j  ,k) +
     &                       thicku(i,j-1,k)*scuy(i,j-1)*u(i,j-1,k)  )
                fluxv = 0.5*(thickv(i,j  ,k)*scvx(i,j  )*v(i,j  ,k) +
     &                       thickv(i,j-1,k)*scvx(i,j-1)*v(i,j-1,k)  )
                sum2(jlat) = sum2(jlat) +
     &            (cos(pang(i,j))*fluxv + sin(pang(i,j))*fluxu) / cross
*               if     (jlat.eq.0) then
*                 write(6,'(a,4i5,3f8.2)')
*    &             'i,j,jlat,k,cross,pang,flux = ',
*    &              i,j,jlat,k,cross,pang(i,j),
*    &              (cos(pang(i,j))*fluxv +
