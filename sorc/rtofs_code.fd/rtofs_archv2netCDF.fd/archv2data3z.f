...
c
     ...
     &          ctrc_lname(99)*80,ctrc_sname(99)*80
      logical   lcell,ltheta,smooth,icegln,lperiod,baclin,xyward
c
      integer          artype,iexpt,iversn,kkin,yrflag,mxlflg
      real             bot,zbot,dudxdn,dudxup,dvdydn,dvdyup
...
c --- 'mltio ' = mix. lay. thick. I/O unit (0 no I/O), choose 2 of 4 kinds:
...
c --- 'densml' =      density jump across mixed-layer (kg/m3, 0 no I/O)
...
c --- 'tthio ' = density          I/O unit (0 no I/O)
     ...
     &             'tempml','("blkinr: ",a6," =",f11.4," degC")')
      if     (i.eq.1) then !'tmljmp'
        tmljmp = qqin
        call blkinr3(qqin,i,
     &               'densml','("blkinr: ",a6," =",f11.4," kg/m3")',
     &               'tmljmq','("blkinr: ",a6," =",f11.4," degC")',
     &               'tempml','("blkinr: ",a6," =",f11.4," degC")')
        if     (i.eq.1) then !'densml'
          densml = qqin
          tmljmq = 0.0
          tempml = 0.0
        elseif (i.eq.2) then !'tmljmq'
          densml = 0.0
          tmljmq = qqin
          tempml = 0.0
        else  !'tempml'
...
c
     ...
     &                 iexpt,iversn,yrflag,kkin)     ! hycom input
          ...
          call getdats(flnm,time3,artype,initl,icegln,trcout,
...
c ---             flux form for better results from mean archives
     ...
     &                          dpv1*v(i,jp1,k) )/
     &                         max(2.0*dp(i,j,k),dpv0+dpv1)
                  utilk(i,j,k)=0.5*sqrt( uvp**2 + vvp**2 )
                else
                  utilk(i,j,k)=utilk(i,j,k-1)
                endif
              else
...
c
c ---   --------------------------------------
c ---   eqivalent temperature mixed layer, PLM
c ---   --------------------------------------
c
...
c
c ---   --------------------------------------
c ---   eqivalent temperature mixed layer, PQM
c ---   --------------------------------------
c
