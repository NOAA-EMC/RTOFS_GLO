...
!
! !REVISION HISTORY:
!  SVN:$Id: ice_history.F90 143 2008-08-08 23:08:22Z eclare $
!
...
!pgp      real (kind=real_kind) :: ltime
      real (kind=real_kind) :: ltime, spval_r
      ...
      if (my_task == master_task) then
        ...
        ! create file
!ajw
        if     (nx_global*ny_global.lt.10000000) then
          status = nf90_create(ncfile, nf90_clobber, ncid)
          if (status /= nf90_noerr) call abort_ice( &
                          'ice: Error creating history ncfile')
        else !too big for 32-bit; requires netcdf 3.6 or above
          status = nf90_create(path=ncfile, &
                               cmode=or(nf90_clobber, &
                                       nf90_64bit_offset), &
                               ncid=ncid)
          if (status /= nf90_noerr) call abort_ice( &
              'ice Error in nf90_create: '//trim(nf90_strerror(status)))
        endif
!ajw - end
...
!ajw
        if (days_per_year.eq.366)then
          write(title,'(a,a,a,a,a,a,a,a)') 'days since ',&
            '1900-12-31 00:00:00' !NRL gregorian
        else
          write(title,'(a,a,a,a,a,a,a,a)') 'days since ', &
...
!pgp            status = nf90_put_att(ncid,varid,'missing_value',spval)
            status = nf90_put_att(ncid,varid,'missing_value',spval_r)
            if (status /= nf90_noerr) call abort_ice( &
                 'Error defining mising_value for '//vname(n))
!pgp            status = nf90_put_att(ncid,varid,'_FillValue',spval)
            status = nf90_put_att(ncid,varid,'_FillValue',spval_r)
            if (status /= nf90_noerr) call abort_ice( &
      ...
      !-----------------------------------------------------------------
            ...
            if (status /= nf90_noerr) call abort_ice( &
                 'Error defining time_rep for '//vname(n))
          endif
        enddo
...
1000    format('This dataset was created on ', &
      ...
      !-----------------------------------------------------------------
        ...
        if (status /= nf90_noerr) call abort_ice( &
              'ice Error in nf90_enddef: '//trim(nf90_strerror(status)))
...
!
      ...
      use ice_restart, only: lenstr, runid
      use ice_itd, only: c_hi_range
      use ice_calendar, only: write_ic, dayyr, days_per_year, histfreq
!
! !INPUT/OUTPUT PARAMETERS:
!
!EOP
...
!ajw
        if (days_per_year == 360) then
           write (nu_hdr, 999) 'calendar','360_day',' '
           write (title,'(a,i3,a)') 'All years have exactly ',int(dayyr),' days'
        elseif (days_per_year == 365) then
           write (nu_hdr, 999) 'calendar','noleap',' '
           write (title,'(a,i3,a)') 'All years have exactly ',int(dayyr),' days'
        else !days_per_year == 366
           write (nu_hdr, 999) 'calendar','standard',' '
           write (title,'(a,i3,a)') 'Years have 365 or 366 days'
        endif
!ajw - end
...
!=======================================================================
      ...
      integer (kind=int_kind) :: iyear, imonth, iday, isec
        ...
        if(my_task.eq.master_task)then
           write(nu_diag,*)"in construct_filename in ice_history"
           write(nu_diag,*)idate
           write(nu_diag,*)iyear,imonth,iday
           write(nu_diag,*)write_ic,hist_avg,histfreq
           write(nu_diag,*)new_year,new_month,new_day
        endif
...
!ajw - end
      ...
      if(my_task.eq.master_task)then
      write(nu_diag,*)" ncfile before ending construct_filename"
      write(nu_diag,*)ncfile
      endif
