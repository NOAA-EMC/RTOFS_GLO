...
!
! !REVISION HISTORY:
!  SVN:$Id: ice_grid.F90 143 2008-08-08 23:08:22Z eclare $
!
...
!
      ...
      type(ESMF_DistGrid) :: iceDistGrid
#endif
...
!ajw
      !-----------------------------------------------------------------
      ! Get global KMT array used for block decomposition.
      !-----------------------------------------------------------------
      ...
      !-----------------------------------------------------------------
      ! Get global ULAT array used for block decomposition, if needed.
      !-----------------------------------------------------------------
...
! !INTERFACE:
!
!pgp      subroutine init_grid2
      subroutine init_grid2(gridComp)
!
! !DESCRIPTION:
!
...
!pgp
      elseif (trim(grid_type) == 'hycom') then
         call hycom_grid(gridComp) ! hycom grid
      else
...
!pgp
      if (grid_type == 'hycom') then
!     HYCOM input includes ANGLE, assumes ANGLE is not used.
      else
      out_of_range = .false.
...
! !INTERFACE:
!
subroutine hycom_grid(gridComp)
!
! !DESCRIPTION:
!
...
!
! --- Create an ESMF grid that matches the HYCOM/CICE 2D T-grid
...
!
! --- make a dist grid object using the deBlockList defined in ice_blocks.
      iceDistGrid=ESMF_DistGridCreate( &
                       minIndex=(/        1,        1/), &
      ...
      if(my_task.eq.master_task)then
        write(nu_diag,*)"After reading grid: errorCode ="
        write(nu_diag,*)errorCode, ESMF_Success
      endif

      if (errorCode /= ESMF_Success) then
         write(nu_diag,*)&
              '(cice) Error in hycom_grid: DistGridCreate'
         return
      endif
!
! --- Now create a 2D grid
      iceGrid=ESMF_GridCreate( &
                     distGrid=iceDistGrid, &
      ...
      if (errorCode /= ESMF_Success) then
         write(nu_diag,*)&
              '(cice) Error in hycom_grid: GridCreate'
         return
      endif

!
! --- Get VM from gridComp
      call ESMF_GridCompGet(gridComp, vm=vm, rc=errorCode)

      if (errorCode /= ESMF_Success) then
         write(nu_diag,*) &
              '(cice) Error in hycom_grid: GridCompGet'
         return
      endif
!
!  Create a DE layout to match HYCOM layout
      deLayout = ESMF_DELayoutCreate(vm, rc=errorCode)

      if (errorCode /= ESMF_Success) then 
         write(nu_diag,*) &
              '(cice) Error in hycom_grid: DELayout'
         return
      endif
!
!  Associate grid with ESMF gridded component
      call ESMF_GridCompSet(gridComp, grid=iceGrid, rc=errorCode)

      if (errorCode /= ESMF_Success) then
         write(nu_diag,*) &
              '(cice) Error in hycom_grid: GridCompSet'
         return
      endif
...
!EOP
      ...
      do j = 1, ny_global
      do i = 1, nx_global
!pgp         work_g(i,j) = work_g(i,j) * cm_to_m                ! HTN
         work_g(i,j) = work_g(i,j)                 ! HTN
      enddo
      enddo
      do j = 1, ny_global
      do i = 1, nx_global
...
!EOP
      ...
      do j = 1, ny_global
      do i = 1, nx_global
!pgp         work_g(i,j) = work_g(i,j) * cm_to_m                ! HTE
         work_g(i,j) = work_g(i,j)                ! HTE
      enddo
      enddo
      do j = 1, ny_global-1
         do i = 1, nx_global
...
!
      ...
      if (my_task==master_task) then
         write(nu_diag,*) ' '
!pgp         write(nu_diag,*) 'min/max ULON:', y1*rad_to_deg, y2*rad_to_deg
!pgp         write(nu_diag,*) 'min/max TLON:', x1*rad_to_deg, x2*rad_to_deg
!pgp         write(nu_diag,*) 'min/max ULAT:', y3*rad_to_deg, y4*rad_to_deg
!pgp         write(nu_diag,*) 'min/max TLAT:', x3*rad_to_deg, x4*rad_to_deg
