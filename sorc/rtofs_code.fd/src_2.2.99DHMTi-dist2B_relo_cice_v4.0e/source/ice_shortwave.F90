...
!
      ...
      ! allow for ice albedo already applied to input radiation (see atm_netrad)
      real (kind=dbl_kind) :: &
         albnetrad   ! ice albedo in input rad, 0.0 (the default) to disable
...
!
      ...
      !-----------------------------------------------------------------
      ! Allow for albedo already applied to the input radiation (if any)
      !-----------------------------------------------------------------

      if (albnetrad > c0 ) then
         call modify_albedos (nx_block,   ny_block, &
                              alvdrni,    alidrni,  &
                              alvdfni,    alidfni,  &
                              alvdrns,    alidrns,  &
                              alvdfns,    alidfns,  &
...
!BOP
!
! !IROUTINE: modify_albedos - allow for ice albedo applied to input radiation
!
! !INTERFACE:
!
      subroutine modify_albedos (nx_block, ny_block, &
                                alvdrni,  alidrni,  &
                                alvdfni,  alidfni,  &
                                alvdrns,  alidrns,  &
                                alvdfns,  alidfns,  &
...
! !DESCRIPTION:
!
! Allow for albnetrad, the ice albedo already in netrad
!
! !REVISION HISTORY:
!
...
!
      ...
      do j = 1, ny_block
      do i = 1, nx_block
         if (albnetrad == c1 ) then
            alvdrn(i,j) = c0
            alidrn(i,j) = c0
            alvdfn(i,j) = c0
            alidfn(i,j) = c0
         else
            alvdrn(i,j) = c1 - (c1 - max( albnetrad, alvdrn(i,j) )) &
                             / (c1 -      albnetrad               )
            alidrn(i,j) = c1 - (c1 - max( albnetrad, alidrn(i,j) )) &
                             / (c1 -      albnetrad               )
            alvdfn(i,j) = c1 - (c1 - max( albnetrad, alvdfn(i,j) )) &
                             / (c1 -      albnetrad               )
            alidfn(i,j) = c1 - (c1 - max( albnetrad, alidfn(i,j) )) &
                             / (c1 -      albnetrad               )
         endif
