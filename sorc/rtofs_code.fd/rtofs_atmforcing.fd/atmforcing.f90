PROGRAM atmforcing
  ...
  !                (READ IN mod_geom.f90)
  !     UNIT 51  - GFS GRIBBED FLUXES FILES WITH THE NAMES FROM THE LIST 
...
!     CALL getgds(51,TRIM(atmnames(m)),gds)
     
     jdiscno=0
     call rdgrib(51,TRIM(atmnames(m)),xgfld,kpds,jpdtno,jdiscno,0,xpts,ypts)
     ...
     atmgrid_changed: IF (atm_grid_changed) THEN
        ...
        CALL mask_atm(52,TRIM(atmnames(1)),kpds567(:,natm),mskfrac,nextrap_max,msk_in &
...
!       write(*,*)'"BEFORE 2nd rdgrib: xpts,ypts=',xpts,ypts
       CALL rdgrib(52+m,TRIM(atmnames(m)),xgfld,kpds,jpdtno,jdiscno,1,xpts,ypts)  
