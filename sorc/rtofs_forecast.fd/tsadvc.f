...
c  on return, fld's valid halo will be 0 wide.
c
      real,    parameter :: onemu= 0.0098  !very small layer thickness
c
...
c
      margin = mbdy_a - 3
c
!$OMP PARALLEL DO PRIVATE(j,l,i,fqmax,fqmin,famax,famin,qp,qm)
!$OMP&         SCHEDULE(STATIC,jblk)
      do j=1-margin,jj+margin
        do l=1,isp(j)
          do i=max(1-margin,ifp(j,l)),min(ii+margin,ilp(j,l))
     ...
     &                             fmnlo(i,ja),fmnlo(i,jb))
            famax = ( max(0.0,fax(ia,j)) - min(0.0,fax(ib,j)) +
     &                max(0.0,fay(i,ja)) - min(0.0,fay(i,jb))  )*dt2 +
     &              epsil
            famin = ( max(0.0,fax(ib,j)) - min(0.0,fax(ia,j)) +
     &                max(0.0,fay(i,jb)) - min(0.0,fay(i,ja))  )*dt2 +
     &              epsil
            qp = (fqmax-fldlo(i,j)) *fcn(i,j)*scal(i,j)
            qm = (fldlo(i,j)-fqmin) *fcn(i,j)*scal(i,j)
            if (qp > 0.0) then
