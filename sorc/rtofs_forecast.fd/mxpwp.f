...
cdiag     if (i.eq.itest .and. j.eq.jtest) then
cdiag       write(6,103) nstep,i+i0,j+j0,k,iter,rig(k)
cdiag       call flush(lp)
cdiag     endif
 103      format('rig(k)',i9,2i5,2i3,1p,e13.5)
        enddo
c
c --- identify interface where rig has a vertical minimum at each grid point
        ...
        enddo
c
c --- if selected layer pair is unstable, mix to bring rig up to rigc+0.05
        if(rig2.lt.rigc) then
          k=kintf
          rig1=1.-rig2/(rigc+0.05)
c
...
cdiag     if (i.eq.itest .and. j.eq.jtest) then
cdiag       write(6,104) nstep,i+i0,j+j0,k,rigf,rig1,v1d(k-1),v1d(k)
cdiag       call flush(lp)
cdiag     endif
 104      format('rig mix',i9,2i5,i3,1p,4e13.5)
c
        end if
c
      enddo
