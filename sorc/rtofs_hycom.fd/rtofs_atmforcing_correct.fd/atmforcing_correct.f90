PROGRAM atmforcing_correct
  ...
  USE mod_hycomio1,ONLY : mask_hycom_1,write_abrecord
  !
  IMPLICIT NONE
  REAL, PARAMETER :: T_f=-1.8 ! Freezing temperature
  REAL :: fldmin,fldmax,time,span
  ...
  READ(5,'(a)') labela
  READ(5,'(a)') labels
  WRITE (*,*) " ******* atmforcing_correct: started"
  ...
  records: DO num=1,num_records
     ...
     END WHERE
     ! dbgz: Ilya: remove correction for HYCOM with CICE
     T_a_star=T_a
     !
     ...
     !     &       '   '//'airtmp',time,fldmin,fldmax
     CALL write_abrecord(T_a_star,imsk,.TRUE.,lua,lub,.FALSE. &
                ,'airtmp',ftime=time,span=span)
     !
  ENDDO records
  !
  CLOSE(lub)
  ...
  DEALLOCATE(ip)
  DEALLOCATE(preambl)
  WRITE (*,*) " ******* atmforcing_correct: finished"
  !
