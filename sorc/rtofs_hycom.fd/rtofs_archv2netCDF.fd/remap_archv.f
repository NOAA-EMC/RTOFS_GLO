...
c
     ...
     &                 ds0k(99),ds0kf,dsm,dsms,dssk(99)
...
c
     ...
     &                                         idm,jdm,')'
      ...
      jorign = 1
c
c --- 'newtop' = kdmnew-kdmold iff only new layers added at top (0 otherwise)
c ---            deepest kdmold target densities and d[ps]00* unchanged
...
c
      if     (newtop.ne.0 .and. newtop.ne.kkout-kkin) then
        write(lp,'(/ a,i3 /)')
     &    'error - newtop must be 0 or kdmnew-kdmold'
        call flush(lp)
      endif
      if     (newtop.lt.0) then
        write(lp,'(/ a,i3 /)')
     &    'error - newtop must be 0 when kdmnew < kdmold'
        call flush(lp)
      endif
c
...
c ---       minimum output depths (sigma-Z only).
            pout(i,j,1) = 0.0
            do k= 1,kkout-1
              dp0ij=min(dp0k(k),max(ds0k(k),dssk(k)*depths(i,j)))
              pout(i,j,k+1) = min(pout(i,j,k) + dp0ij,
     &                            depths(i,j))
c
c ---         preserve isopycnic interior when all the new layers are 
c ---         added to the the top of an otherwise identical configuration.
              if (newtop.gt.0) then
                k2 = k-newtop
                if     (k2.ge.1) then
                  pout(i,j,k+1) = max(pout(i,j,k+1),p(i,j,k2+1))
*                 if     (k.le.nsigma .and.
*    &                    dssk(k)*depths(i,j).lt.dp0k(k)) then
*                   pout(i,j,k+1) = min(pout(i,j,k+1),
*    &                                  pout(i,j,k)+dssk(k)*depths(i,j))
*                 endif
                endif !k2>=1
              endif  !newtop>0
            enddo !k
