...
c
     ...
     &   depth1, depthu,depthv,dpdx,dpdy, util1,work
      real, allocatable, dimension (:,:,:) ::
     &   utilk,w
      real ubi,ubmi,vbi,vbim1,strmft,strmfu,s1,s2
c
...
c
     ...
     &          ctrc_lname(99)*80,ctrc_sname(99)*80
      logical   smooth,mthin,icegln,lperiod,baclin,xyward,barouv
c
      logical          ltheta
            ...
            do i=1,ii
              if (ip(i,j).ne.0) then  !may be approximate at i=ii and j=jj
                ...
                if     (xyward .or. pang(i,j).eq.0.0) then
                  util1(i,j)=       0.5*(ubaro(i,j)+ubaro(ip1,j))
                else
c ---             Rotate from Xward and Yward to Eastward
                  util1(i,j)=
     &              cos( pang(i,j))*0.5*(ubaro(i,j)+ubaro(ip1,j)) +
     &              sin(-pang(i,j))*0.5*(vbaro(i,j)+vbaro(i,jp1))
                endif !pang
              else
                util1(i,j)=flag
              endif
        ...
        if (ioin.gt.0) then
          do j=1,jj
            ...
            do i=1,ii
              if (ip(i,j).ne.0) then  !may be approximate at i=ii and j=jj
                if     (xyward .or. pang(i,j).eq.0.0) then
                  util1(i,j)=       0.5*(vbaro(i,j)+vbaro(i,jp1))
                else
c ---             Rotate from Xward and Yward to Eastward
                  if     (i.ne.ii) then
                    ip1 = i+1
                  ...
                  util1(i,j)=
     &              cos( pang(i,j))*0.5*(vbaro(i,j)+vbaro(i,jp1)) -
     &              sin(-pang(i,j))*0.5*(ubaro(i,j)+ubaro(ip1,j))
                endif !pang
              else
                util1(i,j)=flag
              endif
        ...
        do j=1,jj
          do i=1,ii
            if (ip(i,j).ne.0 .and. i.lt.ii .and. j.lt.jj) then
              ...
              if     (s1.ne.0.0) then
                util1(i,j)=s2/s1 * 1.e6  !Sv to m3/s
              else
                util1(i,j)=flag
              endif
            else
...
c
              util1(i,j)=1.
              if (iu(i,j).eq.1) then
                ubaro(i,j)=u(i,j,k)*.5*(dp(i,j,k)+dp(im1,j,k))
              else
                ubaro(i,j)=0.0
              endif
              if (iv(i,j).eq.1) then
                vbaro(i,j)=v(i,j,k)*.5*(dp(i,j,k)+dp(i,jm1,k))
              else
                vbaro(i,j)=0.0
              endif
              if (iq(i,j).eq.1) then
          ...
          do j=1,jj
            do i=1,ii
              if (ip(i,j).ne.0 .and. i.lt.ii .and. j.lt.jj) then
                ...
                if     (s1.ne.0.0) then
                  utilk(i,j,k)=s2/s1 * 1.e6  !Sv to m3/s
                else
                  utilk(i,j,k)=flag
                endif
              else
