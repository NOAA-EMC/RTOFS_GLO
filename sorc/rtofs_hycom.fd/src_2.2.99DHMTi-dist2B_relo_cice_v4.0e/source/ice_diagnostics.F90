...
!
! !REVISION HISTORY:
!  SVN:$Id: ice_diagnostics.F90 144 2008-08-12 21:37:19Z eclare $
!
...
!
      ...
      use ice_constants
      use ice_calendar, only: diagfreq, istep1, istep, idate, sec
      use ice_fileunits
!
!EOP
!
...
!          Cecilia M. Bitz, UW
!
! 2013: date stamp added by Alan J. Wallcraft
!
! !USES:
!
      use ice_broadcast
...
!ajw
               psstocn(n)   = sstocn(i,j,iblk)       ! ocean SST
...
!ajw
            call broadcast_scalar(psstocn  (n), pmloc(n)) 
...
!ajw
      write(d_s,"(i8,'-',i2.2,' ')") idate,(sec+1800)/3600  ! "yyyymmdd-hh "

      if (my_task == master_task) then
       write(nu_diag,*) '---------------------------------'
       if (grid_type == 'panarctic') then   ! Arctic only
        write (nu_diag,799) 'Arctic diagnostics'
        write (nu_diag,801) d_s,'total ice area  (km^2) = ',arean
        write (nu_diag,801) d_s,'total ice extent(km^2) = ',extentn
        write (nu_diag,801) d_s,'total ice volume (m^3) = ',shmaxn
        write (nu_diag,801) d_s,'total snw volume (m^3) = ',snwmxn
        write (nu_diag,801) d_s,'tot kinetic energy (J) = ',ketotn
        write (nu_diag,800) d_s,'rms ice speed    (m/s) = ',urmsn
        write (nu_diag,800) d_s,'average albedo         = ',albtotn
        write (nu_diag,800) d_s,'max ice volume     (m) = ',hmaxn
        write (nu_diag,800) d_s,'max ice speed    (m/s) = ',umaxn
        write (nu_diag,900) d_s,'max strength    (kN/m) = ',pmaxn
        ...
        if (prescribed_ice) then
          ...
          write (nu_diag,*) '---------------------------------'
          write (nu_diag,801) d_s,'arwt rain h2o kg in dt = ',rnn
          write (nu_diag,801) d_s,'arwt snow h2o kg in dt = ',snn
          write (nu_diag,801) d_s,'arwt evap h2o kg in dt = ',evpn
          write (nu_diag,801) d_s,'arwt frzl h2o kg in dt = ',frzn
          write (nu_diag,801) d_s,'arwt frsh h2o kg in dt = ',sfreshn
         
          write (nu_diag,801) d_s,'arwt ice mass (kg)     = ',micen
          write (nu_diag,801) d_s,'arwt snw mass (kg)     = ',msnwn

          write (nu_diag,801) d_s,'arwt tot mass (kg)     = ',mtotn
          write (nu_diag,801) d_s,'arwt tot mass chng(kg) = ',delmin
          write (nu_diag,801) d_s,'arwt water flux        = ',fluxn
          if (update_ocn_f) then
            write (nu_diag,*) '(=rain+snow+evap-fresh)  '
          else
            write (nu_diag,*) '(=rain+snow+evap+frzl-fresh)  '
          endif
          write (nu_diag,801) d_s,'water flux error       = ',werrn
#if (defined CCSM) || (defined SEQ_MCT)
         endif                    ! prescribed_ice
#endif       
         write (nu_diag,*) '---------------------------------'
         write (nu_diag,801) d_s,'arwt atm heat flux (W) = ',fhatmn
         write (nu_diag,801) d_s,'arwt ocn heat flux (W) = ',fhocnn
         write (nu_diag,801) d_s,'arwt frzl heat flux(W) = ',fhfrzn
         write (nu_diag,801) d_s,'arwt tot energy    (J) = ',etotn
         write (nu_diag,801) d_s,'arwt net heat      (J) = ',hnetn
         write (nu_diag,801) d_s,'arwt tot energy chng(J)= ',delein
         write (nu_diag,801) d_s,'arwt heat error        = ',herrn
       
         write (nu_diag,*) '---------------------------------'
         write (nu_diag,801) d_s,'arwt salt mass (kg)    = ',msltn
         write (nu_diag,801) d_s,'arwt salt mass chng(kg)= ',delmsltn
         write (nu_diag,801) d_s,'arwt salt flx in dt(kg)= ',sfsaltn
         write (nu_diag,801) d_s,'arwt salt flx error    = ',serrn
         write (nu_diag,*) '---------------------------------'
        ...
        write(nu_diag,901) d_s,'total ice area  (km^2) = ',arean,  areas
        write(nu_diag,901) d_s,'total ice extent(km^2) = ',extentn,extents
        write(nu_diag,901) d_s,'total ice volume (m^3) = ',shmaxn, shmaxs
        write(nu_diag,901) d_s,'total snw volume (m^3) = ',snwmxn, snwmxs
        write(nu_diag,901) d_s,'tot kinetic energy (J) = ',ketotn, ketots
        write(nu_diag,900) d_s,'rms ice speed    (m/s) = ',urmsn,  urmss
        write(nu_diag,900) d_s,'average albedo         = ',albtotn,albtots
        write(nu_diag,900) d_s,'max ice volume     (m) = ',hmaxn,  hmaxs
        write(nu_diag,900) d_s,'max ice speed    (m/s) = ',umaxn,  umaxs
        write(nu_diag,900) d_s,'max strength    (kN/m) = ',pmaxn,  pmaxs

        if (print_global) then  ! global diags for conservations checks

         write(nu_diag,*) '---------------------------------'
         write(nu_diag,901) d_s,'arwt rain h2o kg in dt = ',rnn,rns
         write(nu_diag,901) d_s,'arwt snow h2o kg in dt = ',snn,sns
         write(nu_diag,901) d_s,'arwt evap h2o kg in dt = ',evpn,evps
         write(nu_diag,901) d_s,'arwt frzl h2o kg in dt = ',frzn,frzs
         write(nu_diag,901) d_s,'arwt frsh h2o kg in dt = ',sfreshn,sfreshs

         write(nu_diag,901) d_s,'arwt ice mass (kg)     = ',micen,mices
         write(nu_diag,901) d_s,'arwt snw mass (kg)     = ',msnwn,msnws
 
         write(nu_diag,901) d_s,'arwt tot mass (kg)     = ',mtotn,mtots
         write(nu_diag,901) d_s,'arwt tot mass chng(kg) = ',delmin,delmis
         write(nu_diag,901) d_s,'arwt water flux        = ',fluxn,fluxs
         if (update_ocn_f) then
           write (nu_diag,*) '(=rain+snow+evap-fresh)  '
         else
           write (nu_diag,*) '(=rain+snow+evap+frzl-fresh)  '
         endif
         write(nu_diag,901) d_s,'water flux error       = ',werrn,werrs

         write(nu_diag,*) '---------------------------------'
         write(nu_diag,901) d_s,'arwt atm heat flux (W) = ',fhatmn,fhatms
         write(nu_diag,901) d_s,'arwt ocn heat flux (W) = ',fhocnn,fhocns
         write(nu_diag,901) d_s,'arwt frzl heat flux(W) = ',fhfrzn,fhfrzs
         write(nu_diag,901) d_s,'arwt tot energy    (J) = ',etotn,etots
         write(nu_diag,901) d_s,'arwt net heat      (J) = ',hnetn,hnets
         write(nu_diag,901) d_s,'arwt tot energy chng(J)= ',delein,deleis
         write(nu_diag,901) d_s,'arwt heat error        = ',herrn,herrs

         write(nu_diag,*) '---------------------------------'
         write(nu_diag,901) d_s,'arwt salt mass (kg)    = ',msltn,mslts
         write(nu_diag,901) d_s,'arwt salt mass chng(kg)= ',delmsltn, &
                                                        delmslts
         write(nu_diag,901) d_s,'arwt salt flx in dt(kg)= ',sfsaltn, &
                                                        sfsalts
         write(nu_diag,901) d_s,'arwt salt flx error    = ',serrn,serrs
         write(nu_diag,*) '---------------------------------'
      ...
      !-----------------------------------------------------------------
       ...
       if (print_points) then
        ...
        write(nu_diag,*) '            ----------atm----------'
        write(nu_diag,900) d_s,'air temperature (C)    = ',pTair(1),pTair(2)
        write(nu_diag,900) d_s,'specific humidity      = ',pQa(1),pQa(2)
        write(nu_diag,910) d_s,'snowfall (mm)          = ',pfsnow(1), &
                                                           pfsnow(2)
        write(nu_diag,910) d_s,'rainfall (mm)          = ',pfrain(1), &
                                                           pfrain(2)
        if (.not.calc_Tsfc) then
           write(nu_diag,900) d_s,'total surface heat flux= ',pfsurf(1),pfsurf(2)
           write(nu_diag,900) d_s,'top sfc conductive flux= ',pfcondtop(1), &
                                                              pfcondtop(2)
           write(nu_diag,900) d_s,'latent heat flx        = ',pflat(1),pflat(2)
        else
           write(nu_diag,900) d_s,'shortwave radiation sum= ',pfsw(1),pfsw(2)
           write(nu_diag,900) d_s,'longwave radiation     = ',pflw(1),pflw(2)
        endif
        write(nu_diag,*) '            ----------ice----------'
        write(nu_diag,900) d_s,'area fraction          = ',paice(1),paice(2)
        write(nu_diag,900) d_s,'avg ice thickness (m)  = ',hiavg(1),hiavg(2)
        write(nu_diag,900) d_s,'avg snow depth (m)     = ',hsavg(1),hsavg(2)
        if (calc_Tsfc) then
           write(nu_diag,900) d_s,'surface temperature(C) = ',pTsfc(1),pTsfc(2)
           write(nu_diag,900) d_s,'absorbed shortwave flx = ',pfswabs(1), &
                                                              pfswabs(2)
           write(nu_diag,900) d_s,'outward longwave flx   = ',pflwout(1), &
                                                              pflwout(2)
           write(nu_diag,900) d_s,'sensible heat flx      = ',pfsens(1), &
                                                              pfsens(2)
           write(nu_diag,900) d_s,'latent heat flx        = ',pflat(1),pflat(2)
        endif
        write(nu_diag,910) d_s,'subl/cond (mm ice)     = ',pevap(1),pevap(2)
        write(nu_diag,910) d_s,'top melt (mm)          = ',pmeltt(1) &
                                                          ,pmeltt(2)
        write(nu_diag,910) d_s,'bottom melt (mm)       = ',pmeltb(1) &
                                                          ,pmeltb(2)
        write(nu_diag,910) d_s,'lateral melt (mm)      = ',pmeltl(1) &
                                                          ,pmeltl(2)
        write(nu_diag,910) d_s,'new ice (mm)           = ',pfrazil(1), &
                                                           pfrazil(2)
        write(nu_diag,910) d_s,'congelation (mm)       = ',pcongel(1), &
                                                           pcongel(2)
        write(nu_diag,910) d_s,'snow-ice (mm)          = ',psnoice(1), &
                                                           psnoice(2)
        write(nu_diag,910) d_s,'effective dhi (mm)     = ',pdhi(1),pdhi(2)
        write(nu_diag,910) d_s,'effective dhs (mm      = ',pdhs(1),pdhs(2)
        write(nu_diag,900) d_s,'intnl enrgy chng(W/m^2)= ',pde (1),pde (2)
        write(nu_diag,*) '            ----------ocn----------'
!ajw
        write(nu_diag,900) d_s,'sss (psu)              = ',psss(1),psss(2)
        write(nu_diag,900) d_s,'sst - freezing (C)     = ',psst(1)-pTf(1), &
                                                           psst(2)-pTf(2)
        write(nu_diag,900) d_s,'ocean sst - frz (C)    = ',psstocn(1)-pTf(1), &
                                                           psstocn(2)-pTf(2)
!ajw - end
        write(nu_diag,900) d_s,'freezing temp (C)      = ',pTf(1),pTf(2)
        write(nu_diag,900) d_s,'heat used (W/m^2)      = ',pfhocn(1), &
                                                           pfhocn(2)
        write(nu_diag,*) '            -----------------------'
       endif                    ! print_points
  ...
  799 format (39x,a18)
  800 format (a12,a25,2x,f18.11)
  801 format (a12,a25,2x,1pe18.11)
  899 format (39x,a18,2x,a18)
  900 format (a12,a25,2x,f18.11,2x,f18.11)
  910 format (a12,a25,2x,3pf18.11,2x,3pf18.11) !m to mm
  901 format (a12,a25,2x,1pe18.11,2x,1pe18.11)
  902 format (12x,a25,5x,f7.2,1x,f7.2,5x,f7.2,1x,f7.2)
  903 format (12x,a25,2x,i5,1x,i2,1x,i4,1x,i4,2x,i5,1x,i2,1x,i4,1x,i4)
!ajw - end
