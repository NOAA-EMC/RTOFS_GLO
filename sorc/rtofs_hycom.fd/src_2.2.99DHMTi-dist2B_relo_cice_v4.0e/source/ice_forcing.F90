...
!pgp
      logical (kind = log_kind) :: &
         atm_netrad                  ! if true, atm provides netrad (vs glbrad)
                                     ! see also zero_albedo in ice_shortwave
...
#ifdef USE_ESMF
!pgp
      elseif (trim(atm_data_type) == 'nogaps') then
         call nogaps_files(fyear)
      elseif (trim(atm_data_type) == 'cfsr') then
         call nogaps_files(fyear)
#endif
...
!pgp
      if (trim(sst_data_type) == 'nogaps' .or. &
          trim(sss_data_type) == 'nogaps') then
           call ocn_data_nogaps_init
           sss(:,:,:) = 34.0_dbl_kind
      endif
!pgp
      if (trim(sst_data_type) == 'cfsr' .or. &
          trim(sss_data_type) == 'cfsr') then
           call ocn_data_nogaps_init
           sss(:,:,:) = 34.0_dbl_kind
      endif
...
!pgp      ftime = time         ! forcing time
      ftime = time-time_year         ! forcing time
      time_forc = ftime    ! for restarting

      if (my_task.eq.master_task)write(nu_diag,*)ftime,time_forc
      if (my_task.eq.master_task)write(nu_diag,*)trim(atm_data_type)
...
!pgp 
      elseif (trim(sst_data_type) == 'nogaps' .or. &
              trim(sss_data_type) == 'nogaps' ) then
         call ocn_data_nogaps(dt)
!pgp 
      elseif (trim(sst_data_type) == 'cfsr' .or. &
              trim(sss_data_type) == 'cfsr' ) then
         call ocn_data_nogaps(dt)
      endif
...
!pgp
      elseif (trim(atm_data_type) == 'nogaps') then !NOGAPS
      if (yr==0)then
      i = index(data_file,'.r') -6
      tmpname = data_file
      write(data_file,'(a,a)')tmpname(1:i),'.r'
      else
      endif
!pgp
!pgp
      elseif (trim(atm_data_type) == 'cfsr') then !CFSR
      if (yr==0)then
      i = index(data_file,'.r') -6
      tmpname = data_file
      write(data_file,'(a,a)')tmpname(1:i),'.r'
...
!         precip_factor = c1    ! mm/sec = kg/m^2 s
      ...
      ! necessary.
      !-----------------------------------------------------------------
!pgp      winds are already rotated to grid
...
!
!  !IROUTINE : nogaps_files - constructs filesnames for NOGAPS atmospheric
!  data
!
!  !INTERFACE:
      subroutine nogaps_files (yr)
!  !DESCRIPTION:
!  None of hte filenames depend on the year
!
!  !REVISION HISTORY:
!
!  Author: Alan J. Wallcraft, NRL (based on esmwf_files)
!
!  USES:
!
!  INPUT/OUTPUT PARAMETERS:
...
!
       ...
       if (atm_netrad) then
        fsw_file =    trim(atm_data_dir)//'netrad.r'
        else
        fsw_file =    trim(atm_data_dir)//'glbrad.r'
       endif

        flw_file =    trim(atm_data_dir)//'lwdflx.r'
        uwind_file =  trim(atm_data_dir)//'wndewd.r' !actually Xward, not Eward
        vwind_file =  trim(atm_data_dir)//'wndnwd.r' !actually Yward, not Nward
        tair_file  =  trim(atm_data_dir)//'airtmp.r'
        humid_file =  trim(atm_data_dir)//'vapmix.r'
        rain_file =   trim(atm_data_dir)//'prec_lanl_12.r'      !climatology
        rhoa_file =   trim(atm_data_dir)//'rhoa_ncar85-88_12.r' !climatology

        if (my_task.eq.master_task)then
           write(6,*)"atmospheric files = "
           write(6,*)flw_file
           write(6,*)tair_file
           write(6,*)rain_file
           write(6,*)rhoa_file
...
!BOP
!
! !IROUTINE: nogaps_data - read NOGAPS atmospheric data
!
! !INTERFACE:
!
      subroutine nogaps_data
!
! !DESCRIPTION:
!
! Read NOGAPS or CFSR atmospheric data.
!
! !REVISION HISTORY:
!
! authors: Alan J. Wallcraft, NRL
!          Based on ncar_data and ecmwf_data
!
! !USES:
!
      use ice_flux
      use ice_coupling, only: sst_import,sss_import,uocn_import,vocn_import
!
! !INPUT/OUTPUT PARAMETERS:
!
!
...
!
      ...
      initrec         ! initial record number for the year
      ...
      logical (kind=log_kind), save :: &
         at_rec = .false. ! used to delay recnum update by 1 time step

      logical (kind=log_kind) :: readm, read3

      logical, parameter      :: debug_n_d = .true.  !usually false
...
!pgp -99
    ...
    !-------------------------------------------------------------------
    ! 3-hourly data, input files configured for this exact model run
    !-------------------------------------------------------------------

      dataloc = 2          ! data located at end of interval
      sec3hr  = secday/c8  ! seconds in 3 hours
      maxrec  = 9999       ! disabeled

      ! record number in year at end of timestep
      recnum = 8*int(yday) - 7 + int(real(sec)/sec3hr)
!pgp
! added for cfsr
!
      if (trim(atm_data_type) == 'cfsr' ) then
      sec3hr = secday/24.  !really seconds in 1 hr
      recnum = 24*int(yday) - 23 + int(real(sec)/sec3hr)
      endif
!pgp 
      if    (istep/=1 .and. oldrecnum /= recnum) then
         if     (at_rec .and. oldrecnum<recnum) then  !never true for new year
            at_rec = .false.
         else !delay record read for one time step, or forever if a new year
            recnum=oldrecnum
            at_rec = .true.
         endif
      endif
...
!pgp -99
      ...
      if     (.not.at_rec) then  !usual case
        call interp_coeff (recnum, recslot, sec3hr, dataloc)
           if (debug_n_d .and. my_task == master_task) then
             write(nu_diag,'(a,2f8.4,i5,f20.4)') &
              'IC1,2:', c1intp,c2intp,recnum,ftime/secday
           endif                     ! master_task
      else !exactly at record 2
        c1intp = c0
        c2intp = c1
           if (debug_n_d .and. my_task == master_task) then
             write(nu_diag,'(a,2f8.4,i5,f20.4)') &
              'NC1,2:', c1intp,c2intp,recnum,ftime/secday
           endif                     ! master_task
      endif

      !convert from record in year to record in this run

      if     (istep==1) then
        initrec = 2-recnum  !at start, initrec+recnum=2
      endif

      read3 = istep==1 .or. oldrecnum /= recnum


      !Read 3-hrly atmospheric fields



      call read_data (read3, initrec, fyear, ixm, ixx, ixp, maxrec, &
                     tair_file, Tair_data,  &
                     field_loc_center, field_type_scalar)
      ...
      ! Interpolate atmospheric fields
      call interpolate_data (Tair_data, Tair)
      call interpolate_data (uatm_data, uatm)
      call interpolate_data (vatm_data, vatm)
      call interpolate_data ( fsw_data, fsw)
      ...
      if     (ocn_data_dir.ne."COUPLED") then
        !Read 3-hrly ocean fields
        call read_data (read3, initrec, fyear, ixm, ixx, ixp, maxrec, &
                       sst_file, sst_data, &
                       field_loc_center, field_type_scalar)
        ...
        !always lag the ocean fields, for compatibility with coupling
        c1intp = c1
        c2intp = c1 - c1intp
        call interpolate_data (  sst_data, sstocn)
        call interpolate_data (  sss_data,    sss)
        call interpolate_data ( uocn_data,   uocn)
        call interpolate_data ( vocn_data,   vocn)
      else
        !import 3-hrly ocean fields
            
        sstocn(:,:,1) =  sst_import(:,:,1)
           sss(:,:,1) =  sss_import(:,:,1)
          uocn(:,:,1) = uocn_import(:,:,1)
          vocn(:,:,1) = vocn_import(:,:,1)

!pgp  if 1st timestep then initialize values
...
!       endif
      ...
      endif  !ocean read or import

      ! Save record number for next time step
      oldrecnum = recnum


      end subroutine nogaps_data
#endif
...
!=======================================================================
! NOGAPS ocean forcing
!=======================================================================
!
!BOP
!
! !IROUTINE: ocn_data_nogaps_init - reads data set
!
! !INTERFACE:
!
      subroutine ocn_data_nogaps_init
!
! !DESCRIPTION:
!
! Initializes HYCOM ocean forcing data set 
! 
!
! !REVISION HISTORY:
!
! authors: Alan Wallcraft,  NRL
!
! !USES:
!
      use ice_domain, only: nblocks, distrb_info
...
!EOP
!
      if (trim(atm_data_type) == 'nogaps')then
      sss_file = trim(ocn_data_dir)//'ocnsss.r'
      sst_file = trim(ocn_data_dir)//'ocnsst.r'
      uocn_file = trim(ocn_data_dir)//'ocnssu.r'
      vocn_file = trim(ocn_data_dir)//'ocnssv.r'
      endif
      if (trim(atm_data_type) == 'cfsr')then
      sss_file = trim(ocn_data_dir)//'ocnsss.r'
      sst_file = trim(ocn_data_dir)//'ocnsst.r'
      ...
      end   subroutine ocn_data_nogaps_init

!=======================================================================
!BOP
!
! !IROUTINE: ocn_data_nogaps - restores sst
!
! !INTERFACE:
!
      subroutine ocn_data_nogaps(dt)
!
! !DESCRIPTION:
!
...
! !REVISION HISTORY:
!
! author:  Alan Wallcraft, NRL
!
! !USES:
!
      use ice_domain, only: nblocks
      use ice_grid, only: t2ugrid_vector
      use ice_flux, only: Tf,sss,sst,sstocn,uocn,vocn,ss_tltx,ss_tlty
      use ice_diagnostics, only: npnt,latpnt,lonpnt,piloc,pjloc,pbloc,pmloc
...
!
      integer (kind=int_kind) :: &
         i, j, iblk, n      ! horizontal indices

      real (kind=dbl_kind), intent(in) :: &
         dt ! timestep

      real (kind=dbl_kind), parameter :: &
      Trestore= 8.64e4_dbl_kind  ! 1 day
    ...
    !-------------------------------------------------------------------
    ! Note: sss has been restored instantaneously to data.
    ! Make sure sss > 0.
    ! Compute freezing temperature based on salinity
    ! If needed, restore sst to data with time scale Trestore.
    !-------------------------------------------------------------------
      do iblk = 1, nblocks
         do j = 1, ny_block
         do i = 1, nx_block
            sss(i,j,iblk) = max(sss(i,j,iblk),c0)
            Tf (i,j,iblk) = -depressT*sss(i,j,iblk) ! freezing temp, deg
 
            if (restore_sst) then
               sst(i,j,iblk) = sst(i,j,iblk) &
                        + (sstocn(i,j,iblk)-sst(i,j,iblk))*dt/Trestore
!ajw           sst(i,j,iblk) =  max (sst(i,j,iblk), Tf(i,j,iblk))
            endif
         enddo
         enddo
      enddo
    ...
    !-------------------------------------------------------------------
      do n= 1, npnt
         if (my_task == pmloc(n) .and. dbug) then
            i    = piloc(n)
            j    = pjloc(n)
            iblk = pbloc(n)
            write(nu_diag,'(a,i2,2f10.4,2f12.8)') &
               'n,lon,lat,sst-Tf,ocn-Tf =',       &
               n,lonpnt(n),latpnt(n),             &
               sst(   i,j,iblk)-Tf(i,j,iblk),     &
               sstocn(i,j,iblk)-Tf(i,j,iblk)
         endif
      enddo
...
!      call t2ugrid_vector(ss_tlty)
      ...
      end    subroutine ocn_data_nogaps
