...
!
! !REVISION HISTORY:
!  SVN:$Id: ice_fileunits.F90 143 2008-08-08 23:08:22Z eclare $
!
...
!pgp
      ...
      nu_offset = 5000           !unit number offset
!pgp now add offset to all files

      integer (kind=int_kind) :: &
         nu_grid=11+nu_offset       , &  ! grid file
         nu_kmt=12+nu_offset       , &  ! land mask file
         nu_nml=21+nu_offset        , &  ! namelist input file
         nu_forcing=49+nu_offset    , &  ! forcing data file
         nu_dump=50+nu_offset       , &  ! dump file for restarting
         nu_restart=50+nu_offset    , &  ! restart input file
         nu_dump_age=53+nu_offset   , &  ! dump file for restarting ice age tracer
         nu_restart_age=54+nu_offset, &  ! restart input file for ice age tracer
         nu_dump_pond  , &  ! dump file for restarting melt pond tracer
         nu_restart_pond,&  ! restart input file for melt pond tracer
         nu_rst_pointer=52+nu_offset, &  ! pointer to latest restart file
         nu_history=55+nu_offset    , &  ! binary history output file
         nu_hdr=56+nu_offset        , &  ! header file for binary history output
