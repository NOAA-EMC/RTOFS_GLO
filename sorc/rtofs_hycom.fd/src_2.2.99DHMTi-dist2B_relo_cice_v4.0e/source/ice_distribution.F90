...
!
! !REVISION HISTORY:
!  SVN:$Id: ice_distribution.F90 141 2008-08-04 21:45:46Z eclare $
!
...
!***********************************************************************
!BOP
! !IROUTINE: create_deBlockList
! !INTERFACE:

 subroutine create_deBlockList(distribution)

! !DESCRIPTION:
!  This routine determines global block locations for ESMF
!  Only works if there is exactly one block per processor
!
! !REVISION HISTORY:
!  same as module
!
...
!-----------------------------------------------------------------------
   ...
   do jblock=1,nblocks_y
      ...
      do iblock=1,nblocks_x
         ...
         np = distribution%blockLocation(n)
         deBlockList(1,1,np) = is
         deBlockList(1,2,np) = ie
         deBlockList(2,1,np) = js
         deBlockList(2,2,np) = je
...
!   dbug = .false.
   if (dbug) then
      ...
      write(nu_diag,*) 'deBlockList: is,ie,js,je'
      do n = 1, nblocks_tot
         write(nu_diag,'(i5,2x,4i5)') n, &
                                      deBlockList(1,1,n), &
                                      deBlockList(1,2,n), &
                                      deBlockList(2,1,n), &
                                      deBlockList(2,2,n)
...
!EOC
 ...
 end subroutine create_deBlockList
#endif
...
!  requested.  Currently four distributions are supported:
!  2-d Cartesian distribution x then y (cartesian-xy or cartestian),
!  2-d Cartesian distribution y then x (cartesian-yx),
!  a load-balanced distribution using a rake algorithm based on an 
!  input amount of work per block (rake),
!  and a space-filling-curve algorithm (spacecurve).
!
! !REVISION HISTORY:
!  same as module
...
!----------------------------------------------------------------------
   ...
   case('cartesian-xy')  !same as 'cartesian'

      create_distribution = create_distrb_cart(nprocs, work_per_block)

   case('cartesian-yx')

      create_distribution = create_distrb_cartyx(nprocs, work_per_block)
...
!----------------------------------------------------------------------
   ...
   elseif (processor_shape == 'slenderX2') then ! 2 proc in y direction
      jguess = 2
      iguess = nprocs/jguess
   elseif (processor_shape == 'slenderX3') then ! 3 proc in y direction
      jguess = 3
      iguess = nprocs/jguess
   else                                         ! 4 proc in y direction
      jguess = 4
      iguess = nprocs/jguess
   endif
...
!----------------------------------------------------------------------
   ...
   if (processor_shape == 'square-pop') then
      ...
      if (iguess*jguess == nprocs) then ! valid decomp
         ...
         !*** assume slenderX? is a good decomposition
         else if (processor_shape == 'slenderX1' .or. &
                  processor_shape == 'slenderX3' .or. &
                  processor_shape == 'slenderX3' .or. &
                  processor_shape == 'slenderX4') then
            nprocs_x = iguess
            nprocs_y = jguess
            exit proc_loop
...
!***********************************************************************
!BOP
! !IROUTINE: create_distrb_cartyx
! !INTERFACE:

 function create_distrb_cartyx(nprocs, workPerBlock) result(newDistrb)

! !DESCRIPTION:
!  This function creates a distribution of blocks across processors
!  using a 2-d Cartesian distribution, first x then y.
!
! !REVISION HISTORY:
!  same as module
...
!----------------------------------------------------------------------
   ...
   do j=1,nprocsY
   do i=1,nprocsX
      processor = (i-1)*nprocsY + j    ! number the processors 
...
!EOC
 ...
 end function create_distrb_cartyx
