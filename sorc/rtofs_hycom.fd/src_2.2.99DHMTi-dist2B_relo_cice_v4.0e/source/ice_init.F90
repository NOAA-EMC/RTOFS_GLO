...
!ajw
      use ice_ocean, only: oceanmixed_ice, no_oceanalbedo
...
!pgp          oceanmixed_file, restore_sst,   trestore 
         oceanmixed_file, restore_sst,atm_netrad,  trestore 
...
!ajw
                               albnetrad, shortwave, albedo_type, &
                               R_ice, R_pnd, R_snw
...
!pgp
      real (kind=dbl_kind) :: &
        tdayf,                &  !HYCOM firest day of run
        tdayl                    !HYCOM last day of run
...
!pgp      histfreq='m'           ! output frequency option
      histfreq='d'           ! output frequency option
      histfreq_n = 1         ! output frequency
!ajw      hist_avg = .true.      ! if true, write time-averages (not snapshots)
      hist_avg = .false.     ! if true, write time-averages (not snapshots)
!pgp      history_dir  = ' '     ! write to executable dir for default
      history_dir  = './ '     ! write to executable dir for default
!pgp      history_file = 'iceh'  ! history file name prefix
      history_file = './cice'  ! history file name prefix
!pgp      history_format = 'bin' ! file format ('bin'=binary or 'nc'=netcdf)
...
!pgp      incond_file = 'iceh_ic'! file prefix
      incond_file = './cice'! file prefix
!pgp      dumpfreq='y'           ! restart frequency option
!pgp      dumpfreq='m'           ! restart frequency option
      dumpfreq='d'           ! restart frequency option
...
!ajw
      albnetrad = 0.00_dbl_kind   ! ice albedo already in input radiation
      atmbndy   = 'default'       ! or 'constant'

!pgp      fyear_init = 1900          ! first year of forcing cycle
      fyear_init = 1901           ! first year of forcing cycle
...
!pgp
      atm_netrad      = .false.   ! if true, input is netrad (not glbrad)
...
!pgp
          if (npt.eq.-1) then !HYCOM-specific option
            open (nu_nml, file='limits',status='old') !read HYCOM file
            read (nu_nml, *)tdayf,tdayl
            close(nu_nml)
            npt = nint( (tdayl-abs(tdayf))*(secday/dt) )
          endif
...
!ajw
      call broadcast_scalar(albnetrad,          master_task)
...
!pgp
      call broadcast_scalar(atm_netrad,         master_task)
      ...
      call broadcast_scalar(insert_ssmi,        master_task)
...
!ajw
      no_oceanalbedo = atm_netrad  !albedo is already in netrad
...
!ajw
         write(nu_diag,1000) ' albnetrad                 = ', albnetrad
