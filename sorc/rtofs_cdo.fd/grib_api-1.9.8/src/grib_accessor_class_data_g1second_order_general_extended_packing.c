...
#define MAX_NUMBER_OF_GROUPS 450000
#define EFDEBUG 0
...
static long value_count(grib_accessor* a)
{
  ...
  if(grib_get_long_internal(a->parent->h,self->number_of_values,&numberOfCodedValues) != GRIB_SUCCESS)
       return 0;

  return numberOfCodedValues;
}
...
static int pack_double(grib_accessor* a, const double* val, size_t *len)
{
	...
	if((ret = grib_set_long_internal(a->parent->h,self->decimal_scale_factor, 0))
     != GRIB_SUCCESS)
		return ret;
	...
	*/
	...
	if (numberOfGroups > 65535 ) {
		extraValues=(numberOfGroups+65535)/65536;	
