...
#   this script runs 2dvar pre_QC and NCODA QC for AMSR
...
cd $log_dir
cat amsr_*.$cut_dtg > acspo_sst_files.${ddtg}_prelim
...
if [[ ! -f acspo_sst_files.$ddtg || ! -s acspo_sst_files.$ddtg ]]; then
   echo "WARNING - acspo_sst_files.$ddtg does not exist/is empty. No AMSR files to process."
   echo "AMSR.obs_control file will not be updated"
fi

#   execute ncoda pre_qc for AMSR netCDF files
$EXECrtofs/rtofs_ncoda_acspo_sst_nc amsr $ddtg $upd > amsr_preqc.$ddtg.out
...
#   execute ncoda qc
ln -s $OCN_DATA_DIR/incoming/amsr.a.$ddtg $OCN_DATA_DIR/incoming/amsr.a
ln -s $OCN_DATA_DIR/incoming/amsr.b.$ddtg $OCN_DATA_DIR/incoming/amsr.b
$EXECrtofs/rtofs_ncoda_qc $ddtg amsr_sst > amsr_qc.$ddtg.out
err=$?; export err ; err_chk
echo " error from rtofs_ncoda_qc=",$err
mv fort.44 amsr_qc.$ddtg.rej
