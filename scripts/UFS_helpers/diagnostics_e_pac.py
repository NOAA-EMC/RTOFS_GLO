...
get_inputs = ArgumentParser(description="\
           Calculate eastern Pacific (lat: [-17, 12], lon: [130W, 70W] approximately)\
           mean and std dev of a 2-d (globally defined) field \
...
region = 'e_Pac'  # This script is meant for East Pacific diagnostics and plots.
...
[x_id_ep_s, x_id_ep_e] = [2000, 2700]
[y_id_ep_s, y_id_ep_e] = [1300, 1650]

ds = ds_glb.sel(X=slice(x_id_ep_s, x_id_ep_e),\
                Y=slice(y_id_ep_s, y_id_ep_e))
...
# print(f'\nPlotting {args.varName}...\n')
  ...
  cbar=plt.colorbar(im, ax=ax, pad=0.05, orientation=cbar_orientation, shrink=0.75)
